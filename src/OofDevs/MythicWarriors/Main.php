...
class Main extends PluginBase implements Listener {
    ...
    public function OnDamage(EntityDamageByEntityEvent $event) {
        ...
        if ($player instanceof Player) {
            ...
            $tag = $attacker->getNameTag();
            $race = $this->getTitanRace($tag);
            $damage = $this->getRaceDamage($race);
            $player->setHealth($player->getHealth() - $damage);
            $racep = $this->getRace($attacker);
            $class = $this->getClass($race);
            if ($class == "FlameLord") {
                $player->setOnFire(3);
                $damage = $this->getRaceDamage($racep);
            ...
            if ($sender->hasPermission("mythic.create")) {
                if ($sender instanceof Player) {
                    if (isset($args[1])) {
                        ...
                        $sender->sendMessage($this->Msg("Please set the race and charecter name!"));
                    }
                } else {
            ...
            if ($sender->hasPermission("mythic.createrace")) {
                if ($sender instanceof Player) {
                    if (isset($args[7])) {
                        ...
                        $sender->sendMessage($this->Msg("Please set the race, size, damage, health, hunger, level, effect and ability!"));
                    }
                } else {
                    $sender->sendMessage($this->Msg("In-Game only!"));
                }
